...
export function detectFileType(filename: string): FileType {
  ...
  if (!type) {
    throw new UnsupportedFileError(
      `"${filename}" isn't a supported file type. LocalDesk reads PDF, DOCX, CSV/TSV, and TXT files.`,
    );
  }
  return type;
}
...
export function validateFile(file: File): FileType {
  ...
  if (file.size > MAX_FILE_SIZE_BYTES) {
    throw new FileTooLargeError(
      `"${file.name}" is ${(file.size / 1024 / 1024).toFixed(1)}MB, which is over LocalDesk's 25MB per-file limit.`,
    );
  }
  return type;
}
